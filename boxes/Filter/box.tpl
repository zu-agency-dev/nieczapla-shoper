                            ...
                            {foreach from=$group.items item=item}
                                {if $item.active or (isset($item.counter) and $item.counter) or !isset($item.counter)}
                                    <li class="{if $item.active}selected fa fa-times{/if} {if $item.color}color-filter{/if}">
                                        <a title="{$item.name|escape}" href="{if $item.active}{$item.link_remove}{else}{if $filter_type == 1 || !$item.link_add}{$item.link_solo}{else}{$item.link_add}{/if}{/if}"{if $item.color} class="color-filter"{/if} {if $item.link_part}data-link-part="{$item.link_part}"{/if} {$boxNs->$box_id->link_rel}>
