  ...
  <header class="row">
    ...
    <div class="logo-bar row container">
      <a href="{baseDir nonempty=1}" title="{translate key='Home page'}"
        class="link-logo {if $seo_has_blank_logo}link-logo-text{else}link-logo-img{/if}">
      ...
      {if 1 == $skin_settings->header->basket}
        ...
        <div class="basket-contain">
            ...
            <ul class="basket-product-list">
              {foreach from=$user->basket item=basket_product}
                {if !$basket_product->isChild()}
                  <li data-product-id="{$basket_product->product->product->product_id}"
                    data-category="{$basket_product->product->defaultCategory->translation->name|escape}"
                    {if $basket_product->product->product->producer_id}data-producer="{$basket_product->product->producer->manufacturer->name|escape}"
                    {/if}>
                    <img
                      src="{imageUrl type='productGfx' width=75 height=75 image=$basket_product->stock->mainImageName() overlay=1}"
                      alt="{$basket_product->product->translation->name|escape}" />

                    <a class="product-name"
                      href="{route function='product' key=$basket_product->product->product->product_id productName=$basket_product->product->translation->name productId=$basket_product->product->product->product_id}"
                      title="{$basket_product->product->translation->name|escape}">
                    ...
                    <span class="product-info">
                      <span
                        class="product-amount">{float precision=$QUANTITY_PRECISION value=$basket_product->basket->quantity trim=true}</span>
                      x <span class="product-price">{currency value=$basket_product->getPrice()}</span>
                    </span>

                    <span class="remove-product"><a
                        href="{route key='basketRemove' basketId=$basket_product->getIdentifier()}">{translate key="remove"}</a></span>
                  </li>
                {/if}
              {/foreach}
            </ul>
          ...
          <div class="basket-summery">
            ...
            <span
              class="basket-price{if $user->basket->sumProducts(false, false) !== $user->basket->sumProducts()} basket-price-discount{/if}">
      ...
      {if $searchWithSuggest == false}
        <form class="search-form right {if 1 != $skin_settings->header->searchbox}none{/if}" action="{route key='search'}"
          method="post">
          <fieldset>
            <input type="text" name="search" placeholder="{translate key='search in the store...'}" value=""
              class="search-input s-grid-3" />
            <button type="submit" class="search-btn btn btn-red">
              <img src="{baseDir}/libraries/images/1px.gif" alt="" class="px1">
              <span>{translate key="Search"}</span>
            </button>
            <a href="{route key='search'}" title="{translate key='advanced search'}"
              class="none adv-search">{translate key='advanced search'}</a>
          </fieldset>
        </form>
      {else}
      ...
      <div class="innerbreadcrumbs row container">
        ...
        <ul itemscope itemtype="https://schema.org/BreadcrumbList" class="path left inline">
          ...
          {foreach from=$breadcrumbs->getBreadCrumbs() item=item name=bclist}
            <li class="bred-{$smarty.foreach.bclist.iteration} {if $smarty.foreach.bclist.last}last{/if}"
              {if $item.url}itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem" {/if}>
        ...
        {foreach from=$banners item=banner}
          {if 'swf' == substr($banner->banner->file, -3)}
            ...
            <object type="application/x-shockwave-flash"
              data="{baseDir}/{$banner->getUrl()}?clickTag={$banner->clickTagLink()|escape}"
              width="{$banner->banner->width|escape}" height="{$banner->banner->height|escape}"
              title="{$banner->banner->name|escape}">
            ...
            <!--[if IE]>
                            ...
                            <![endif]-->
          {else}
            <a {if 1 == (int) $banner->banner->blank}target="_blank" rel="noopener" {/if}
              href="{route key='banner' bannerId=$banner->getIdentifier()}" title="{$banner->banner->name|escape}"
              class="banner">
              <img src="{baseDir}/{$banner->getUrl()}" alt="{$banner->banner->name|escape}"
                width="{$banner->banner->width|escape}" height="{$banner->banner->height|escape}" />
            </a>
          {/if}
        {/foreach}
