{* TODO: isComposable *}
<div class="box tab" id="box_bundle" data-composing="0" data-composing-limit="5"
	...
	<div class="innerbox">
		{foreach from=$product->bundle->stocks item=bundleStock key=k}
			...
			<div data-bundle-id="{$bundleStock->getIdentifier()}" data-stock-id="{$item->getIdentifier()}"
				data-has-variants="{if $item->product->product->group_id && count($options)}1{else}0{/if}" class="product row">
				<div class="f-row">
					<h3 class="rwd-show-medium rwd-hide-full">
						<a href="{route function='product' key=$item->product->getIdentifier() productName=$item->product->translation->name productId=$item->product->getIdentifier()}"
					...
					<div>
						<a href="{imageUrl type='productGfx' image=$item->mainImageName()}"
							title="{$item->product->translation->name|escape}{if $item->isOption()} ({$item->getName()|escape}){/if}"
							{if $skin_settings->productdetails->galleryversion == constant('Logic_SkinVariant::GALLERY_OLD')}
								data-gallery="true"
							data-gallery-list="{$item->product->translation->name|escape}-{$bundleStock->getIdentifier()}" {/if}
							...
							<img
								...
								{if $skin_settings->productdetails->galleryversion == constant('Logic_SkinVariant::GALLERY_NEW')}
									class="js__open-gallery" data-gallery-name="productBundle{$bundleStock->getIdentifier()}Gallery"
							...
							{if count($item->product->galleryGfxs) > 1}
								<span {if $skin_settings->productdetails->galleryversion == constant('Logic_SkinVariant::GALLERY_NEW')}
									data-gallery-name-to-open="productBundle{$bundleStock->getIdentifier()}Gallery" {/if}>
							...
							<div class="none">
								{foreach from=$item->product->galleryGfxs item=img}
									{if $img->product->mainImageId() != $img->gfx->gfx_id}
										<a href="{imageUrl type='productGfx' image=$img->gfx->unic_name}" title="{$img->translation->name|escape}"
											{if $skin_settings->productdetails->galleryversion == constant('Logic_SkinVariant::GALLERY_OLD')}
												data-gallery="true"
											data-gallery-list="{$item->product->translation->name|escape}-{$bundleStock->getIdentifier()}" {else}
											class="js__gallery-anchor-image" {/if}>
											<img
												...
												data-image-description="{$img->translation->name|escape}" {/if} />
										</a>
									{/if}
								{/foreach}
							</div>
						...
						<div class="f-grid-10">
							<a class="details row rwd-hide-medium rwd-hide-small"
								...
								title="{$item->product->translation->name|escape}{if $item->isOption()} ({$item->getName()|escape}){/if}">
								<span class="productname">{$item->product->translation->name|escape}{if $item->isOption()}
									({$item->getName()|escape}){/if}</span>
							</a>
							...
							<p class="r--spacing-xs">
								{translate key="Quantity in bundle"}: <span data-quantity>{$bundleStock->stock->stock|escape}</span>
								{$item->product->unit->translation->name|escape}
							</p>
												...
												<div class="stock-control">
													<div class="stock-label">
														{if $option.type == 'checkbox'}
															<div
																class="form-control form-control-checkbox variant option-{$option.type|escape}{if 1 == $option.stock} option-truestock{else} option-stock{/if}{if 1 == $option.required} option-required{/if}">
																<input data-variant-name="{$value.name|escape}" type="checkbox"
																	id="option-{$item->getIdentifier()}-{$option.id|escape}-{$bundleStock->getIdentifier()}"
																	name="option_{$item->getIdentifier()}_{$option.id|escape}_{$bundleStock->getIdentifier()}"
																	value="1" data-variant-id="{$option.id|escape}">
																<label data-yes="{translate key="YES"}" data-no="{translate key="NO"}"
																	for="option-{$item->getIdentifier()}-{$option.id|escape}-{$bundleStock->getIdentifier()}"></label>
															</div>
														{/if}
														...
														<span class="stock-name{if 1 == $option.required} stock-required{/if}">
															{if $option.type == 'checkbox'}<label
																	for="option-{$item->getIdentifier()}-{$option.id|escape}-{$bundleStock->getIdentifier()}">{/if}{$option.name|escape}{if $option.type == 'checkbox'}</label>{/if}
															</span>
														...
														{if $option.type != 'checkbox'}
															<div class="stock-options">
																<div
																	class="form-control variant option-{$option.type|escape}{if 1 == $option.stock} option-truestock{else} option-stock{/if}{if 1 == $option.required} option-required{/if}">
																	{if  $option.type == 'file'}
																		<input type="file"
																			id="option-{$item->getIdentifier()}-{$option.id|escape}-{$bundleStock->getIdentifier()}"
																			name="option_{$item->getIdentifier()}_{$option.id|escape}_{$bundleStock->getIdentifier()}"
																			data-variant-id="{$option.id|escape}">
																	...
																	{elseif  $option.type == 'radio' || $option.type == 'select'}
																		{foreach from=$option.values item=value}
																			<span class="radio-wrap">
																				<input data-variant-name="{$value.name|escape}" type="radio"
																					id="option-{$item->getIdentifier()}-{$option.id|escape}-{$value.id|escape}-{$bundleStock->getIdentifier()}"
																					name="option_{$item->getIdentifier()}_{$option.id|escape}_{$bundleStock->getIdentifier()}"
																					value="{$value.id|escape}" data-variant-id="{$option.id|escape}"
																					data-variant-value="{$value.id|escape}">
																				<label
																					for="option-{$item->getIdentifier()}-{$option.id|escape}-{$value.id|escape}-{$bundleStock->getIdentifier()}"
																					class="radio-wrap__label">{$value.name|escape}</label>
																			</span>
																			<label
																				for="option-{$item->getIdentifier()}-{$option.id|escape}-{$value.id|escape}-{$bundleStock->getIdentifier()}"
																				class="radio-wrap__label">{$value.name|escape}</label> <br />
																		{/foreach}
																	{elseif  $option.type == 'color'}
																		{foreach from=$option.values item=value}
																			<span data-color="{$value.color|escape}" data-variant-name="{$value.name|escape}"
																				class="product-option_color" data-stock-option title="{$value.name|escape}"
																				data-variant-id="{$option.id|escape}" data-variant-value="{$value.id|escape}"
																				style="background-color: {$value.color|escape}"></span>
																		{/foreach}
																	{/if}
																</div>
															</div>
