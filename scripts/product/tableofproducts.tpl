    ...
    {if 'full' == $view || $view == 'desc'}
        <div class="products view{$view}" {if $price_mode == '2'}data-netto="true"{/if}>
            {foreach from=$products item=product name=prodlist}                    
                <div data-product-id="{$product->product->product_id}" data-category="{$product->defaultCategory->translation->name|escape}" {if $product->product->producer_id}data-producer="{$product->producer->manufacturer->name|escape}"{/if} class="oneperrow f-row product product_view-extended {$columnCounter}{if $product->readVisibilityConfigFlag($listItemGrayFlag)} product_inactive{/if}">
                        ...
                        <div>
                            <a class="details f-grid-4 prodimage" href="{route function=$productRoute key=$product->product->product_id productName=$product->translation->name productId=$product->product->product_id}" title="{$product->translation->name|escape}" rel="dofollow">
                                <span class="f-grid-12 img-wrap lazy-load">
                                    <img src="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs%3D" width="{imageWidth gfx=$product->getPromotingGfx() thumbnailSize=$skin_settings->img->medium}" height="{imageHeight gfx=$product->getPromotingGfx() thumbnailSize=$skin_settings->img->medium}" data-src="{imageUrl type='productGfx' width=$skin_settings->img->medium height=$skin_settings->img->medium image=$product->getPromotingGfxName() overlay=1}" alt="{$product->translation->name|escape}">

                                    <noscript>
                                        <img src="{imageUrl type='productGfx' width=$skin_settings->img->medium height=$skin_settings->img->medium image=$product->getPromotingGfxName() overlay=1}" width="{imageWidth gfx=$product->getPromotingGfx() thumbnailSize=$skin_settings->img->medium}" height="{imageHeight gfx=$product->getPromotingGfx() thumbnailSize=$skin_settings->img->medium}" alt="{$product->translation->name|escape}">
                                    </noscript>
                                </span>
                            </a>
                                ...
                                {if ($enable_availability_notifier && $product->isEnabledNotifier()) || ($enablebasket && $product->canBuyStock())}
                                    {if $enablebasket && $product->canBuyStock()}
                                        <fieldset>
                                            ...
                                            {else}
                                                <a href="{route function=$productRoute key=$product->product->product_id productName=$product->translation->name productId=$product->product->product_id}" class="btn btn-red">{if $loyalty_exchange}{translate key="Exchange"}{else}{translate key="Add to cart"}{/if}</a>
                                            {/if}
                                        </fieldset>
    ...
    {elseif 'phot' == $view}
        <div class="products products_extended view{$view} s-row" {if $price_mode == '2'}data-netto="true"{/if}>
            {foreach from=$products item=product name=prodlist}
                <div data-product-id="{$product->product->product_id}" data-category="{$product->defaultCategory->translation->name|escape}" {if $product->product->producer_id}data-producer="{$product->producer->manufacturer->name|escape}"{/if} class="product product_view-extended s-grid-3 product-main-wrap{if $additional_tax_info == '1'} additional-info{/if}{if $unit_price_calculation} unit-price-info{/if}{if $product->readVisibilityConfigFlag($listItemGrayFlag)} product_inactive{/if}{if $showLowestPriceHistory && $isLowestPriceVisible} product_with-lowest-price{/if}{if $product->currency and $currency->getIdentifier() != $product->currency->getIdentifier()} product_with-currency{/if}">
                    <div class="product-inner-wrap">
                        <a class="prodimage f-row" href="{route function=$productRoute key=$product->product->product_id productName=$product->translation->name
                            ...
                            <span class="f-grid-12 img-wrap{if $product->galleryGfxs|@count > 1} replace-img-list{/if} lazy-load">
                                <img src="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs%3D" width="{imageWidth gfx=$product->getPromotingGfx() thumbnailSize=$skin_settings->img->medium}" height="{imageHeight gfx=$product->getPromotingGfx() thumbnailSize=$skin_settings->img->medium}" data-src="{imageUrl type='productGfx' width=$skin_settings->img->medium height=$skin_settings->img->medium image=$product->getPromotingGfxName() overlay=1}" alt="{$product->translation->name|escape}" {foreach from=$product->galleryGfxs item=img name=gal}
                                ...
                                <noscript>
                                    ...
                                    image=$product->getPromotingGfxName() overlay=1}" width="{imageWidth gfx=$product->getPromotingGfx() thumbnailSize=$skin_settings->img->medium}" height="{imageHeight gfx=$product->getPromotingGfx() thumbnailSize=$skin_settings->img->medium}" alt="{$product->translation->name|escape}" />
                                </noscript>
                            </span>
                        </a>
                        ...
                        <div class="product__basket">
                            ...
                            <a class="btn large tablet quickview" {if $enablebasket}data-price="true"{/if} data-eval="{$skin_settings->productdetails->score}" data-id="{$product->product->product_id}">
                                <img class="px1" alt="" src="{baseDir}/libraries/images/1px.gif"><span>{translate key="check more"}</span>
                            </a>
                        </div>
                        </div>
                    </div>
